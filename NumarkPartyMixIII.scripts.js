...
NumarkPartyMixIII.init = function() {
    ...
    engine.beginTimer(50, () => {
        NumarkPartyMixIII.leftDeck.setDeckStartLeds();
        NumarkPartyMixIII.rightDeck.setDeckStartLeds();
    }, true);
    ...
    engine.makeConnection("[App]", fadeFxIndicatorControl, function() {
        if (isFadeFxOn) {
            if (engine.getValue("[App]", fadeFxIndicatorControl) === 0) {
                NumarkPartyMixIII.led.setFadefxDim();
                NumarkPartyMixIII.led.setFadefxShiftDim();
            } else {
                NumarkPartyMixIII.led.setFadefxBright();
                NumarkPartyMixIII.led.setFadefxShiftBright();
            }
        } else {
            NumarkPartyMixIII.led.setFadefxDim();
            NumarkPartyMixIII.led.setFadefxShiftDim();
...
NumarkPartyMixIII.shutdown = function() {
    ...
    NumarkPartyMixIII.led.setAllShiftOff();
};
...
NumarkPartyMixIII.Deck = function(deckIndex, deckNumber) {
    ...
    const pflConnection = engine.makeConnection(group, "pfl", function() {
        if (engine.getValue(group, "pfl") === 1) {
            NumarkPartyMixIII.led.setPflBright(deckIndex);
        } else {
            NumarkPartyMixIII.led.setPflDim(deckIndex);
        }
    });
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        this.hotcueConnections[i] = engine.makeConnection(group, hotcueStatusControl, function() {
            if (currentPadMode === 0) {
                ...
                if (hotCueStatus === 0) {
                    NumarkPartyMixIII.led.setPadOff(i, deckIndex);
                    NumarkPartyMixIII.led.setPadShiftOff(i, deckIndex);
                } else { //status = 1 or 2
                    NumarkPartyMixIII.led.setPadBright(i, deckIndex);
                    NumarkPartyMixIII.led.setPadShiftBright(i, deckIndex);
                }
            }
        });
    }
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        engine.makeConnection(effectRackEffectUnitGroup, effectEnableControl, function() {
            if (engine.getValue(effectRackEffectUnitGroup, effectEnableControl) === 1) {
                ...
                NumarkPartyMixIII.led.setPadDim(i, deckIndex);
            }
        });
    }
    ...
    engine.makeConnection(group, beatloopRollControl, function() {
        if (engine.getValue(group, beatloopRollControl) === 1) {
            NumarkPartyMixIII.led.setPadBright(4, deckIndex);
        } else {
            NumarkPartyMixIII.led.setPadDim(4, deckIndex);
        }
    });
    ...
    const syncConnection = engine.makeConnection(group, "sync_enabled", function() {
        ...
        if (syncIndicatorState === 0) {
            NumarkPartyMixIII.led.setSyncDim(deckIndex);
        } else if (syncIndicatorState === 1) {
            NumarkPartyMixIII.led.setSyncBright(deckIndex);
        }
    });
    ...
    const cueConnection = engine.makeConnection(group, "cue_indicator", function() {
        ...
        if (cueIndicatorState === 0) {
            NumarkPartyMixIII.led.setCueDim(deckIndex);
            NumarkPartyMixIII.led.setCueShiftDim(deckIndex);
        } else if (cueIndicatorState === 1 || cueIndicatorState === 2) {
            NumarkPartyMixIII.led.setCueBright(deckIndex);
            NumarkPartyMixIII.led.setCueShiftBright(deckIndex);
        }
    });
    ...
    const playConnection = engine.makeConnection(group, "play", function() {
        ...
        if (playState === 0) {
            NumarkPartyMixIII.led.setPlayDim(deckIndex);
            ...
            NumarkPartyMixIII.led.setPlayBright(deckIndex);
        }
    });
    ...
    const playAndCueShiftTrackLoadedConnection = engine.makeConnection(group, "track_loaded", function() {
        ...
        if (loadedState === 0) {
            NumarkPartyMixIII.led.setCueShiftDim(deckIndex);
            NumarkPartyMixIII.led.setPlayShiftDim(deckIndex);
        } else if (loadedState === 1) {
            NumarkPartyMixIII.led.setCueShiftBright(deckIndex);
            NumarkPartyMixIII.led.setPlayShiftBright(deckIndex);
        }
    });
    ...
    const setAllStemPadLeds = function() {
        if (currentPadMode === 4) {
            if (engine.getValue(drumsStemGroup, "mute") === 1) {
                NumarkPartyMixIII.led.setPad1Off(deckIndex);
            } else {
                NumarkPartyMixIII.led.setPad1Bright(deckIndex);
            }
            if (engine.getValue(bassStemGroup, "mute") === 1) {
                NumarkPartyMixIII.led.setPad2Off(deckIndex);
            } else {
                NumarkPartyMixIII.led.setPad2Bright(deckIndex);
            }
            if (engine.getValue(synthsStemGroup, "mute") === 1) {
                NumarkPartyMixIII.led.setPad3Off(deckIndex);
            } else {
                NumarkPartyMixIII.led.setPad3Bright(deckIndex);
            }
            if (engine.getValue(voiceStemGroup, "mute") === 1) {
                NumarkPartyMixIII.led.setPad4Off(deckIndex);
            } else {
                NumarkPartyMixIII.led.setPad4Bright(deckIndex);
            }
        }
    };
    ...
    const stemsStateConnection = engine.makeConnection(group, "stem_count", function() {
        if (engine.getValue(group, "stem_count") > 0) {
            ...
            if (currentPadMode === 4) {
                NumarkPartyMixIII.led.setAllPadsOff(deckIndex);
            }
        }
    ...
    const setAcapelAndInstruLed = function() {
        ...
        if (isStemsTrackLoaded) {
            if (isDrumStemMuted && isBassStemMuted && isSynthsStemMuted && !isVoiceStemMuted) {
                NumarkPartyMixIII.led.setAcapelBright(deckIndex);
                NumarkPartyMixIII.led.setInstruDim(deckIndex);
            } else if (!isDrumStemMuted && !isBassStemMuted && !isSynthsStemMuted && isVoiceStemMuted) {
                NumarkPartyMixIII.led.setAcapelDim(deckIndex);
                NumarkPartyMixIII.led.setInstruBright(deckIndex);
            } else {
                NumarkPartyMixIII.led.setAcapelDim(deckIndex);
                NumarkPartyMixIII.led.setInstruDim(deckIndex);
            }
        } else {
            NumarkPartyMixIII.led.setAcapelOff(deckIndex);
            NumarkPartyMixIII.led.setInstruOff(deckIndex);
        }
    };
    ...
    this.setDeckStartLeds = function() {
        ...
        NumarkPartyMixIII.led.setPflShiftDim(deckIndex); // shift pfl is always dim

        loadConnection.trigger();
        NumarkPartyMixIII.led.setLoadShiftBright(deckIndex); // shift load is always bright
        ...
        NumarkPartyMixIII.led.setSyncShiftDim(deckIndex); //shift sync is always dim (different than serato on purpose)
        ...
        NumarkPartyMixIII.led.setModeLoopsDim(deckIndex);
        NumarkPartyMixIII.led.setModeSamplerDim(deckIndex);
        NumarkPartyMixIII.led.setModeStemsDim(deckIndex);
    };
        ...
        input: function(_channel, _control, value) {
            if (value === 127) {
                ...
                case 0: { // hotcue
                    ...
                    break;
                case 1: // loops
                    NumarkPartyMixIII.led.setModeHotcueDim(deckIndex);
                    NumarkPartyMixIII.led.setModeLoopsBright(deckIndex);

                    NumarkPartyMixIII.led.setAllPadsOff(deckIndex);
                    NumarkPartyMixIII.led.setAllPadsShiftOff(deckIndex);
                    break;
                case 2: // fx
                    NumarkPartyMixIII.led.setModeSamplerBright(deckIndex);

                    NumarkPartyMixIII.led.setAllPadsDim(deckIndex);
                    NumarkPartyMixIII.led.setAllPadsShiftOff(deckIndex);
                    break;
                case 3: // sampler
                    NumarkPartyMixIII.led.setModeLoopsDim(deckIndex);
                ...
                case 1: // loops
                    if (value === 127) {
                        ...
                        if (engine.getValue(group, `beatloop_${beatloopPadSizes[this.padNumber - 1]}_enabled`) === 1) {
                            NumarkPartyMixIII.led.setPadBright(this.padNumber, deckIndex);
                        } else {
                            NumarkPartyMixIII.led.setPadOff(this.padNumber, deckIndex);
                        }
                    }
                    break;
